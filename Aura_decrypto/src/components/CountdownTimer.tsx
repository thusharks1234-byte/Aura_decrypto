...
const CountdownTimer: React.FC<CountdownTimerProps> = ({ targetDate, label, onExpire, compact = false }) => {
  ...
  if (timeLeft.expired) return (
    <span style={{ color: 'var(--error)', fontSize: '0.8rem', fontWeight: 700 }}>EXPIRED</span>
  );
  ...
  return (
    <div>
      {label && <div style={{ fontSize: '0.7rem', color: 'rgba(var(--text-rgb), 0.4)', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 8 }}>{label}</div>}
