...
const ResultsPage: React.FC = () => {
  ...
  const revealedBids = bids
    .filter(b => b.status === 'revealed' || b.status === 'won')
    .sort((a, b) => Number(BigInt(b.revealed_amount_wei || '0') - BigInt(a.revealed_amount_wei || '0')));

  const winner = revealedBids[0];
  const secondPrice = revealedBids[1];
  const pricePaid = auction?.auction_type === 'vickrey' && secondPrice
    ? secondPrice.revealed_amount_wei
    : winner?.revealed_amount_wei;
  ...
  if (!auction) return (
    <div style={{ minHeight: '100vh', background: '#050505', paddingTop: 64, display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: 16 }}>
      ...
      <Link to="/dashboard" style={{ color: '#00ff88' }}>← Dashboard</Link>
    </div>
  );
    ...
    <div style={{ minHeight: '100vh', background: '#050505', paddingTop: 64 }}>
      <div style={{ maxWidth: 900, margin: '0 auto', padding: '40px 24px 80px' }}>
        ...
        <div style={{ marginBottom: 36 }}>
          ...
          <p style={{ color: 'rgba(255,255,255,0.4)' }}>Auction Results & Verification</p>
        </div>
        ...
        {winner ? (
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 16, flexWrap: 'wrap' }}>
              ...
              <div>
                <div style={{ fontSize: '0.75rem', color: 'rgba(255,255,255,0.4)', textTransform: 'uppercase', letterSpacing: '0.12em', marginBottom: 4 }}>Winner</div>
                <div style={{ fontFamily: 'monospace', fontSize: '1.1rem', fontWeight: 700 }}>{shortenAddr(winner.bidder_address)}</div>
              </div>
              <div style={{ marginLeft: 'auto', textAlign: 'right' }}>
                <div style={{ fontSize: '0.75rem', color: 'rgba(255,255,255,0.4)', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 4 }}>
                  {auction.auction_type === 'vickrey' ? 'Price Paid (2nd)' : 'Price Paid'}
                </div>
                <div style={{ fontFamily: 'Outfit, sans-serif', fontSize: '2rem', fontWeight: 800, background: 'linear-gradient(135deg, #00ff88, #00ccff)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>
                  {pricePaid ? weiToEth(pricePaid) : weiToEth(winner.revealed_amount_wei || '0')} ETH
                </div>
                {auction.auction_type === 'vickrey' && (
                  <div style={{ fontSize: '0.8rem', color: 'rgba(255,255,255,0.4)' }}>Bid: {weiToEth(winner.revealed_amount_wei || '0')} ETH</div>
                )}
              </div>
            </div>
          </motion.div>
        ) : (
          <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 16, padding: '24px', marginBottom: 28, textAlign: 'center', color: 'rgba(255,255,255,0.4)' }}>
            {auction.status !== 'ended' ? 'Auction still in progress — results will appear here when it ends.' : 'No valid bids were revealed for this auction.'}
          </div>
        )}

        {/* Leaderboard */}
        <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 16, overflow: 'hidden', marginBottom: 24 }}>
          <div style={{ padding: '16px 20px', borderBottom: '1px solid rgba(255,255,255,0.07)', display: 'flex', alignItems: 'center', gap: 10 }}>
            <Medal size={16} color="#ffcc00" />
              ...
              <motion.div 
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ fontSize: '0.72rem', color: 'rgba(255,255,255,0.3)', marginTop: 2 }}>
                    {bid.revealed_at ? new Date(bid.revealed_at).toLocaleString() : ''}
                  </div>
                </div>
                <div style={{ fontFamily: 'Outfit, sans-serif', fontWeight: 700, fontSize: '1.1rem', color: i === 0 ? '#00ff88' : '#fff' }}>
        ...
        {/* Uncommitted / Unrevealed bids */}
        {bids.filter(b => b.status === 'committed').length > 0 && (
          <div style={{ background: 'rgba(255,204,0,0.05)', border: '1px solid rgba(255,204,0,0.15)', borderRadius: 12, padding: '16px 20px', marginBottom: 24, fontSize: '0.875rem', color: 'rgba(255,204,0,0.8)' }}>
            ⚠ {bids.filter(b => b.status === 'committed').length} bid(s) were committed but never revealed and are considered invalid.
          </div>
        )}

        {/* Refund Panel */}
        <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 16, padding: '24px 20px', marginBottom: 24 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16 }}>
            <RotateCcw size={16} color="#00ccff" />
          ...
          {bids.filter(b => b.status === 'lost' || b.status === 'refund_pending').length === 0 ? (
            <p style={{ color: 'rgba(255,255,255,0.3)', fontSize: '0.875rem' }}>No pending refunds to display.</p>
          ) : (
          ...
          <p style={{ fontSize: '0.75rem', color: 'rgba(255,255,255,0.2)', marginTop: 12 }}>
            Refunds use pull-payment pattern (CEI compliant). In production, call claimRefund() on the smart contract.
          </p>
        </div>

        {/* Verification Panel */}
        <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 16, padding: '24px 20px' }}>
          ...
          <p style={{ fontSize: '0.85rem', color: 'rgba(255,255,255,0.4)', lineHeight: 1.7 }}>
            All commitments and reveals are verifiable on Sepolia testnet. The winner is determined purely by on-chain logic — no off-chain authority.
          </p>
          <a href={`https://sepolia.etherscan.io/address/${auction.contract_address}`} target="_blank" rel="noopener noreferrer" style={{ display: 'inline-block', marginTop: 12, color: '#00ccff', fontSize: '0.875rem', textDecoration: 'none' }}>
